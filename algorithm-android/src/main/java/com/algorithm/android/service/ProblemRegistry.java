...
/**
 * Registry service for all algorithm problems
 * This mirrors the problem registry from console Main.java
 */
public class ProblemRegistry {
    
    private static ProblemRegistry instance;
    private final Map<String, List<ProblemInfo>> categoryMap;
    private final List<ProblemInfo> allProblems;
    ...
    private ProblemRegistry() {
        ...
        try {
            initializeRegistry();
            Log.d("ProblemRegistry", "Registry initialized with " + allProblems.size() + " problems");
        } catch (Exception e) {
            Log.e("ProblemRegistry", "Error initializing registry", e);
            e.printStackTrace();
    ...
    /**
     * Get problem by name or alias
     */
    public ProblemInfo findProblem(String name) {
        for (ProblemInfo problem : allProblems) {
            if (problem.name.equalsIgnoreCase(name)) {
                return problem;
            }
            for (String alias : problem.aliases) {
    ...
    public List<ProblemInfo> searchProblems(String query) {
        ...
        for (ProblemInfo problem : allProblems) {
            // Check name
            if (problem.name.toLowerCase().contains(lowerQuery)) {
                results.add(problem);
                continue;
            }
            ...
            // Check keywords
            for (String keyword : problem.keywords) {
            ...
            // Check description
            if (problem.description.toLowerCase().contains(lowerQuery)) {
                results.add(problem);
            }
        }
    ...
    /**
     * Match problem from spoken problem statement
     * Enhanced to match problem descriptions, clues, and common phrases
     */
    public ProblemInfo matchProblemFromStatement(String statement) {
        if (statement == null || statement.trim().isEmpty()) {
            return null;
        }
        
        String lowerStatement = statement.toLowerCase().trim();
        String[] statementWords = lowerStatement.split("\\s+");
        
        // Score each problem based on multiple criteria
        ...
        for (ProblemInfo problem : allProblems) {
            ...
            // 1. Check if problem name appears (highest priority)
            String lowerName = problem.name.toLowerCase();
            if (lowerStatement.contains(lowerName) || 
                lowerStatement.contains(lowerName.replace(" ", ""))) {
                score += 10;
            }
            
            // Check name word-by-word for partial matches
            String[] nameWords = lowerName.split("\\s+");
            int nameWordMatches = 0;
            for (String nameWord : nameWords) {
                if (nameWord.length() > 2) { // Ignore short words like "a", "an", "the"
                    for (String stmtWord : statementWords) {
                        if (stmtWord.contains(nameWord) || nameWord.contains(stmtWord)) {
                            nameWordMatches++;
                            break;
                        }
                    }
            ...
            if (nameWordMatches > 0) {
                score += nameWordMatches * 2;
            }
            
            // 2. Check aliases
            for (String alias : problem.aliases) {
                String lowerAlias = alias.toLowerCase();
                if (lowerStatement.contains(lowerAlias)) {
                    score += 5;
                }
                // Word-by-word alias matching
                String[] aliasWords = lowerAlias.split("\\s+");
                for (String aliasWord : aliasWords) {
                    if (aliasWord.length() > 2) {
                        for (String stmtWord : statementWords) {
                            if (stmtWord.contains(aliasWord) || aliasWord.contains(stmtWord)) {
                                score += 2;
                                break;
                            }
                        }
            ...
            // 3. Check keywords (weighted by importance)
            for (String keyword : problem.keywords) {
                String lowerKeyword = keyword.toLowerCase();
                if (lowerStatement.contains(lowerKeyword)) {
                ...
                if (keyword.contains(" ")) {
                    ...
                    for (String kwWord : keywordWords) {
                        if (kwWord.length() > 3) {
                            for (String stmtWord : statementWords) {
                                if (stmtWord.contains(kwWord) || kwWord.contains(stmtWord)) {
                                    score += 1;
                                    break;
                                }
                            }
            ...
            // 4. Check short description (very important for matching clues)
            if (problem.shortDescription != null && !problem.shortDescription.isEmpty()) {
                String lowerShortDesc = problem.shortDescription.toLowerCase();
                if (lowerStatement.contains(lowerShortDesc)) {
                    score += 8;
                }
                // Word-by-word short description matching
                String[] shortDescWords = lowerShortDesc.split("\\s+");
                int shortDescMatches = 0;
                for (String descWord : shortDescWords) {
                    if (descWord.length() > 3) {
                        for (String stmtWord : statementWords) {
                            if (stmtWord.contains(descWord) || descWord.contains(stmtWord)) {
                                shortDescMatches++;
                                break;
                            }
                        }
                ...
                if (shortDescMatches >= 2) {
                    score += shortDescMatches * 2;
                }
            }
            
            // 5. Check full description (for detailed problem statements)
            if (problem.description != null && !problem.description.isEmpty()) {
                String lowerDesc = problem.description.toLowerCase();
                
                // Check for key phrases from description
                String[] descSentences = lowerDesc.split("[.!?]");
                for (String sentence : descSentences) {
                    sentence = sentence.trim();
                    if (sentence.length() > 10) {
                        // Count matching words between statement and description sentence
                        String[] descWords = sentence.split("\\s+");
                        int matchingWords = 0;
                ...
                // Check for common problem patterns in description
                if (lowerDesc.contains("given") && lowerStatement.contains("given")) {
                    score += 2;
                }
                if (lowerDesc.contains("return") && lowerStatement.contains("return")) {
                    score += 2;
                }
                if (lowerDesc.contains("find") && lowerStatement.contains("find")) {
                    score += 2;
                }
                if (lowerDesc.contains("check") && lowerStatement.contains("check")) {
                    score += 2;
                }
            }
            
            // 6. Check for common problem statement patterns
            // Examples: "find two numbers", "check if array", "given an array"
            String[] commonPatterns = {
                "two sum", "two numbers", "add up to",
                "contains duplicate", "duplicate", "appears twice",
                "valid anagram", "anagram", "same characters",
                "group anagrams", "group", "anagrams",
                "top k", "k most", "k frequent",
                "product except self", "product", "except",
                "longest consecutive", "consecutive sequence",
                "valid palindrome", "palindrome",
                "longest substring", "substring", "without repeating",
                "reverse string", "reverse",
                "valid parentheses", "parentheses", "brackets",
                "min stack", "stack", "minimum",
                "binary search", "search", "sorted array",
                "climbing stairs", "stairs", "steps",
                "coin change", "coins", "change",
                "longest increasing", "increasing subsequence",
                "word break", "word", "break",
                "house robber", "robber", "houses",
                "number of islands", "islands", "grid",
                "clone graph", "clone", "graph",
                "course schedule", "courses", "schedule",
            ...
            for (String pattern : commonPatterns) {
                if (lowerStatement.contains(pattern)) {
                    // Check if this pattern matches the problem
                    String lowerProblemName = problem.name.toLowerCase();
                    String lowerShortDesc = problem.shortDescription != null ? 
                        problem.shortDescription.toLowerCase() : "";
                    
                    if (lowerProblemName.contains(pattern) || lowerShortDesc.contains(pattern)) {
                        score += 4;
                    }
                }
            }
        ...
        // Only return if we have a reasonable match (score >= 3 to avoid false positives)
        // Lower threshold allows for partial matches from clues
        return bestScore >= 3 ? bestMatch : null;
    }
    
    /**
     * Initialize the problem registry
     * This mirrors getProblemsForCategory from Main.java
     */
    ...
    private void addProblemsToCategory(String category, ProblemInfo[] problems) {
        List<ProblemInfo> list = Arrays.asList(problems);
